...
func (u *Task) CreateTask(ctx core.Context, task *entity.Task) (uint32, error) {
	return task.Id, ctx.DB().Create(task).Error
}

func (u *Task) UpdateTask(ctx core.Context, task *entity.Task) error {
	return ctx.DB().Updates(task).Error
}

func (u *Task) DeleteTask(ctx core.Context, id uint32) error {
	return ctx.DB().Delete(entity.Task{}, "id=?", id).Error
}

func (u *Task) GetTaskValue(ctx core.Context, taskId, userId uint32) (*entity.TaskValue, error) {
	nc := entity.TaskValue{}
	return &nc, ctx.DB().First(&nc, "task_id=? and user_id=?", taskId, userId).Error
}

func (u *Task) ListTaskValue(ctx core.Context, req *types.ListTaskValueRequest) ([]*entity.TaskValue, uint32, error) {
...
func (u *Task) AllTaskValueByTaskId(ctx core.Context, id uint32) ([]*entity.TaskValue, error) {
	var list []*entity.TaskValue
	return list, ctx.DB().Where("task_id=?", id).Find(&list).Error
}

func (u *Task) CreateTaskValue(ctx core.Context, task *entity.TaskValue) (uint32, error) {
	return task.Id, ctx.DB().Create(task).Error
}

func (u *Task) UpdateTaskValue(ctx core.Context, task *entity.TaskValue) error {
	return ctx.DB().Where("user_id = ? and task_id = ?", task.UserId, task.TaskId).Updates(task).Error
}

func (u *Task) DeleteTaskValue(ctx core.Context, id uint32) error {
	return ctx.DB().Delete(entity.TaskValue{}, "id=?", id).Error
}

func (u *Task) FinishTaskValueUsers(ctx core.Context, id uint32) ([]uint32, error) {
	var ids []uint32
