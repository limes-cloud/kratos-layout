...
const OperationBannerCreateBanner = "/partyaffairs.api.banner.Banner/CreateBanner"
const OperationBannerDeleteBanner = "/partyaffairs.api.banner.Banner/DeleteBanner"
const OperationBannerListBanner = "/partyaffairs.api.banner.Banner/ListBanner"
const OperationBannerListVisibleBanner = "/partyaffairs.api.banner.Banner/ListVisibleBanner"
const OperationBannerUpdateBanner = "/partyaffairs.api.banner.Banner/UpdateBanner"
...
type BannerHTTPServer interface {
	...
	// ListVisibleBanner ListVisibleBanner 获取可见轮播图信息列表
...
func RegisterBannerHTTPServer(s *http.Server, srv BannerHTTPServer) {
	r := s.Route("/")
	r.GET("/partyaffairs/api/visible-banners", _Banner_ListVisibleBanner0_HTTP_Handler(srv))
	r.GET("/partyaffairs/api/v1/banners", _Banner_ListBanner0_HTTP_Handler(srv))
...
func _Banner_ListVisibleBanner0_HTTP_Handler(srv BannerHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in emptypb.Empty
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		http.SetOperation(ctx, OperationBannerListVisibleBanner)
		h := ctx.Middleware(func(ctx context.Context, req any) (any, error) {
			return srv.ListVisibleBanner(ctx, req.(*emptypb.Empty))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
type BannerHTTPClient interface {
	...
	ListVisibleBanner(ctx context.Context, req *emptypb.Empty, opts ...http.CallOption) (rsp *ListBannerReply, err error)
...
func (c *BannerHTTPClientImpl) ListVisibleBanner(ctx context.Context, in *emptypb.Empty, opts ...http.CallOption) (*ListBannerReply, error) {
	var out ListBannerReply
	pattern := "/partyaffairs/api/visible-banners"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationBannerListVisibleBanner))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
